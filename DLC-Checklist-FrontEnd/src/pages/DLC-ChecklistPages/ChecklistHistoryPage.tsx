  ...
  const columns: ColumnsType<HistoryDataType> = [
...

    {
      ...
      render:    (text, user) => (
        <>
          {
            user.userRole === 'admin' || user.userRole === 'systemAdmin' ?<Link to={`/SingleUserPage/${user.secret}`}>{user.userName}</Link>: <div>{user.userName}</div>
          }
        </>
      ),
...

    {
      ...
      render: (text, user) =>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          ...
          { decodedToken.userRole === 'admin' || decodedToken.userRole === 'systemAdmin' ?
            <>
