...
import OpenedCamera                                     from './components/CameraComponents/OpenedCamera'
import PhotoView                                        from './components/CameraComponents/PhotoView'
...
const CameraTool = ({ dutiesId }: CameraToolProps) => {
  ...
  return (
    <div style={{display: 'flex',justifyContent: 'center',alignItems: 'center'}}>
      <CameraOutlined style={{fontSize: '20px'}} onClick={() => setShowCamera(true)} />
      {showCamera ? (
        <OpenedCamera
      ...
      {(photo && hasPhoto && !isPhotoDeleted ) || hasOldPhoto && !isPhotoDeleted ? (
        <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
          <EyeOutlined style={{ marginLeft: '15px', fontSize: '20px' }} onClick={() => setShowImage(true)} />
          <DeleteOutlined style={{marginLeft: '15px', fontSize: '20px'}} onClick={deletePhoto}/>
        </div>
      ) : (
        ''
      )}
